...
template CreateOperator(string cmdName, string operator)
{
    const string CreateOperator = "
        floatCommands[\"" ~ cmdName ~ "\"] = function (string path, Context context)
        {
            if (context.size < 2)
            {
...
static this()
{
    ...
    floatCommands["eq"] = function (string path, Context context)
    {
        if (context.size < 2)
        {
            auto msg = "`" ~ path ~ "` expects at least 2 arguments";
    ...
    floatCommands["neq"] = function (string path, Context context)
    {
        if (context.size < 2)
        {
            auto msg = "`" ~ path ~ "` expects at least 2 arguments";
