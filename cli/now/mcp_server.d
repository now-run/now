...
int mcpServer(Document document, string[] documentArgs)
{
    ...
    auto resourceTemplates = mcp.getOrCreate!Dict("resource_templates");
    ...
    foreach (thing, list; dictMap)
    {
        foreach (name, infoItem; list.values)
        {
            ...
            if (thing == "resources")
            {
                entrypoints[info.values["uri"].toString] = new Procedure(name, info);
            }
            else
            {
...
void processRequest(string request, Document document)
{
    ...
    switch (name) {
        ...
        case "resources/templates/list":
            showList("resourceTemplates", request_id);
            break;
...
{
    log("call: ", name, " ", params, " [", request_id, "]");
